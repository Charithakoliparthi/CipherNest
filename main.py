...
dataset = pd.read_csv('/content/kdd_train.csv') 
...
main = tkinter.Tk() 
main.title("Cyber Threat Detection Based on Artificial Neural Networks Using Event Profiles") #designing main screenmain.geometry("1300x1200") 
le = preprocessing.LabelEncoder() 
global filename 
...
def neuralNetwork():
    ...
    #enc = OneHotEncoder()
    #enc.fit(y_trains1)y_train1 = enc.transform(y_trains1)
    #enc = OneHotEncoder()
    #enc.fit(y_tests1)
    #y_test1   = enc.transform(y_tests1)
    enc.fit(y_trains1)
    x_train1 = enc.transform(y_trains1)
    enc = OneHotEncoder()
    enc.fit(y_tests1)
    y_test1 = enc.transform(y_tests1)
    ...
    if lstm_precision < 1:
        ...
        lstm_fm = f1_score(y_test1, prediction_data, average='macro') * 10
...
preprocessButton = Button(main, text="Run Preprocessing TF-IDF Algorithm", 
command=tfidf) 
